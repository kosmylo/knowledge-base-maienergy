...
INDEX_NAME = os.getenv("HOUSEHOLDS_NUMBER_INDEX")
            ...
            index_body = {
                ...
                "mappings": {
                    "properties": {
                        ...
                        "freq": {"type": "keyword"},
                        "agechild": {"type": "keyword"},
                        "n_child": {"type": "keyword"},
                        "hhcomp": {"type": "keyword"},
                        "unit": {"type": "keyword"},
...
csv_file = 'data/numerical/eurostat/households_number.csv'
metadata_file = 'data/numerical/eurostat/households_number_metadata.json'
...
try:
    ...
    for _, row in tqdm(df.iterrows(), total=df.shape[0], desc="Processing Households Number records"):
        try:
            description = metadata.get("description", "")
            geo_time_period = row["geo\\TIME_PERIOD"]
            country_name = country_code_to_name.get(geo_time_period, geo_time_period)

            subject = (f"Households data for household composition {row['hhcomp']} "
                    f"with age of children {row['agechild']} and number of children {row['n_child']} "
