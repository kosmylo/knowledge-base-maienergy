...
INDEX_NAME = os.getenv("INABILITY_KEEP_HOME_WARM_INDEX")
...
csv_file = 'data/numerical/eurostat/inability_to_keep_home_adequately_warm.csv'
metadata_file = 'data/numerical/eurostat/inability_to_keep_home_adequately_warm_metadata.json'
...
for _, row in tqdm(df.iterrows(), total=df.shape[0], desc="Processing Home Warmth Inability records"):
    description = metadata.get("description", "")
    geo_time_period = row["geo\\TIME_PERIOD"]
    country_name = country_code_to_name.get(geo_time_period, geo_time_period)

    subject = (f"Inability to keep home adequately warm for household type '{row['hhtyp']}' "
               f"and income group '{row['incgrp']}' in {country_name} during period {row['period']} ({row['year']}). "
