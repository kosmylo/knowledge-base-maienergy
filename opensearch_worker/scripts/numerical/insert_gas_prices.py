...
INDEX_NAME = os.getenv("GAS_PRICES_INDEX")
...
csv_file = 'data/numerical/eurostat/gas_prices.csv'
metadata_file = 'data/numerical/eurostat/gas_prices_metadata.json'
...
for _, row in tqdm(df.iterrows(), total=df.shape[0], desc="Processing Gas Prices records"):
    description = metadata.get("description", "")
    geo_time_period = row["geo\\TIME_PERIOD"]
    country_name = country_code_to_name.get(geo_time_period, geo_time_period)
    subject = (f"Gas price data for product {row['product']} with consumption band {row['nrg_cons']} "
