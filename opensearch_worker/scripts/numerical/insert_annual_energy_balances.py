...
INDEX_NAME = os.getenv("ANNUAL_ENERGY_BALANCES_INDEX")
...
csv_file = 'data/numerical/eurostat/annual_energy_balances.csv'
metadata_file = 'data/numerical/eurostat/annual_energy_balances_metadata.json'
...
actions = []
for _, row in tqdm(df.iterrows(), total=df.shape[0], desc="Processing Annual Energy Balances records"):
    description = metadata.get("description", "")
    geo_time_period = row["geo\\TIME_PERIOD"]
    country_name = country_code_to_name.get(geo_time_period, geo_time_period)
    subject = (f"Annual energy balances data for balance code {row['nrg_bal']} "
           f"and energy product {row['siec']} in country {country_name} "
