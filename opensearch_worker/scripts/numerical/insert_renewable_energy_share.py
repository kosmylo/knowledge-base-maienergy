...
INDEX_NAME = os.getenv("RENEWABLE_ENERGY_SHARE_INDEX")
...
csv_file = 'data/numerical/eurostat/renewable_energy_share.csv'
metadata_file = 'data/numerical/eurostat/renewable_energy_share_metadata.json'
...
for _, row in tqdm(df.iterrows(), total=df.shape[0], desc="Processing Renewable Energy Share records"):
    description = metadata.get("description", "")
    geo_time_period = row["geo\\TIME_PERIOD"]
    country_name = country_code_to_name.get(geo_time_period, geo_time_period)

    subject = (f"Renewable energy share ({row['nrg_bal']}) "
