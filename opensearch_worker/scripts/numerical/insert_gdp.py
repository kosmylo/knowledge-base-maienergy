...
INDEX_NAME = os.getenv("GDP_INDEX")
...
csv_file = 'data/numerical/eurostat/gdp.csv'
metadata_file = 'data/numerical/eurostat/gdp_metadata.json'
...
for _, row in tqdm(df.iterrows(), total=df.shape[0], desc="Processing GDP records"):
    description = metadata.get("description", "")
    geo_time_period = row["geo\\TIME_PERIOD"]
    country_name = country_code_to_name.get(geo_time_period, geo_time_period)

    subject = (f"GDP data for item {row['na_item']} "
