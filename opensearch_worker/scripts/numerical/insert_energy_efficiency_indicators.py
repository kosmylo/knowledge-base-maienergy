...
INDEX_NAME = os.getenv("ENERGY_EFFICIENCY_INDICATORS_INDEX")
...
csv_file = 'data/numerical/eurostat/energy_efficiency_indicators.csv'
metadata_file = 'data/numerical/eurostat/energy_efficiency_indicators_metadata.json'
...
try:
    ...
    for _, row in tqdm(df.iterrows(), total=df.shape[0], desc="Processing Energy Efficiency Indicators"):
        try:
            description = metadata.get("description", "")
            geo_time_period = row["geo\\TIME_PERIOD"]
            country_name = country_code_to_name.get(geo_time_period, geo_time_period)
            subject = (f"Energy efficiency indicator data for balance code {row['nrg_bal']} "
