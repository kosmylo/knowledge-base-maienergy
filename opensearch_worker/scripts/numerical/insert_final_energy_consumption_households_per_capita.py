...
INDEX_NAME = os.getenv("FINAL_ENERGY_CONSUMPTION_INDEX")
...
csv_file = 'data/numerical/eurostat/final_energy_consumption_households_per_capita.csv'
metadata_file = 'data/numerical/eurostat/final_energy_consumption_households_per_capita_metadata.json'
...
try:
    ...
    for _, row in tqdm(df.iterrows(), total=df.shape[0], desc="Processing Final Energy Consumption records"):
        try:
            description = metadata.get("description", "")
            geo_time_period = row["geo\\TIME_PERIOD"]
            country_name = country_code_to_name.get(geo_time_period, geo_time_period)
            subject = (f"Final energy consumption per capita in country {country_name} "
                    f"during period {row['period']} ({row['year']}). "
