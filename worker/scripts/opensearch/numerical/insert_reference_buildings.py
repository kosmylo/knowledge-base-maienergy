...
INDEX_NAME = os.getenv("REFERENCE_BUILDINGS_INDEX")
...
# Load dataset
csv_file = 'data/numerical/bso/reference_buildings.csv'

try:
    df = pd.read_csv(csv_file).fillna("")

    actions = []
    for _, row in tqdm(df.iterrows(), total=df.shape[0], desc="Processing Reference Buildings records"):
        try:
