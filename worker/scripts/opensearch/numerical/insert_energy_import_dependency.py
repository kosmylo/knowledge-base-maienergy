...
INDEX_NAME = os.getenv("ENERGY_IMPORT_DEPENDENCY_INDEX")
...
csv_file = 'data/numerical/eurostat/energy_import_dependency.csv'
metadata_file = 'data/numerical/eurostat/energy_import_dependency_metadata.json'
...
try:
    ...
    for _, row in tqdm(df.iterrows(), total=df.shape[0], desc="Processing Energy Import Dependency records"):
        try:
            description = metadata.get("description", "")
            geo_time_period = row["geo\\TIME_PERIOD"]
            country_name = country_code_to_name.get(geo_time_period, geo_time_period)
            subject = (f"Energy import dependency for energy product {row['siec']} "
