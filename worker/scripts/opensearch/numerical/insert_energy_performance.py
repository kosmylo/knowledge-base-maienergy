...
INDEX_NAME = os.getenv("ENERGY_PERFORMANCE_INDEX")
...
# Load dataset
csv_file = 'data/numerical/bso/energy_and_environmental_performance.csv'

try:
    df = pd.read_csv(csv_file).fillna("")

    actions = []
    for _, row in tqdm(df.iterrows(), total=df.shape[0], desc="Processing Energy Performance records"):
        try:
